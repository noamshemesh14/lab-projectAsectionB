    ...
    *,
    ...
    for i, q in enumerate(queries):
        ...
        for idx in rerank_idx:
            pid = normalize_page_id(page_ids[int(idx)])
            score = fusion_scores[np.where(dense_candidates == idx)[0][0]]

            if pid not in page_best or score > page_best[pid][1]:
                page_best[pid] = (int(idx), score)

        rerank_idx = np.array(
            [idx for idx, _ in sorted(page_best.values(), key=lambda x: x[1], reverse=True)],
            dtype=np.int64
        )
