...
1. **Index too large / OOM**
   - Reduce `chunk_size` (currently 120 words)
   - Increase `chunk_size` for fewer, larger chunks

2. **NDCG@10 low**
   - Check RRF weights (0.3 / 0.7) – try 0.4 / 0.6 or 0.5 / 0.5
   - Increase `RERANK_POOL` (currently 150) for more cross-encoder candidates
   - Verify cross-encoder model is loaded correctly

3. **Slow at inference**
   - Reduce embedding batch size (currently 64)
   - Profile with `cProfile` to identify bottlenecks

4. **Page deduplication removing good results**
   - If multiple chunks from same page rank highly, only top is kept
   - Consider keeping top-2 per page (modify `retrieve.py` lines 74–85)

---

## 📝 Submission Checklist

- ✅ All Python files included (main.py, index.py, retrieve.py, etc.)
- ✅ `requirements.txt` has all dependencies
- ✅ `artifacts/` folder with 3 files:
  - `bm25_index.pkl`
  - `chunk_vectors.npy`
  - `bm25_meta.json`
- ✅ `data/Wikipedia Entries/` corpus present
- ✅ `data/public_queries.json` test set included
- ✅ This README documenting the pipeline
- ✅ Public GitHub repository (all files pushed)
...
- **Cross-Encoders**: https://www.sbert.net/docs/pretrained-models/cross-encoders.html
...
*Section B Retrieval Pipeline — Lab Project*
